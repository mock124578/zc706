...
// parameters.h
#include "xparameters.h"     // <--- 您刚刚粘贴的那个文件！

// (在您的文件顶部，定义一个全局的 HLS IP 实例)
...
AD9361_InitParam default_init_param = {
    ...
    /* Gain Control */
    1,      //gc_rx1_mode *** RX1增益控制模式：手动增益（0）；Fast AGC（1）；Slow AGC（2）；Hybrid AGC（3）
    1,      //gc_rx2_mode *** RX2增益控制模式：手动增益（0）；Fast AGC（1）；Slow AGC（2）；Hybrid AGC（3）
...
//   ad9361_do_calib(ad9361_phy, RFDC_CAL, 0);
    ...
    xil_printf("\n--- QPSK 混合模式已启用 ---\n");
    xil_printf("  输入任何 '未知' 文本 (例如 'hello') 将作为 QPSK 消息发送。\n");
    xil_printf("----------------------------------------\n");
    ...
    while(1)
    {
        ...
        {
            param_no = 0;
            //console_check_commands 函数只支持以 !、?、= 结尾的命令。
        ...
        if(!command_found)
        {
            xil_printf("命令: 收到未知命令, 尝试作为 QPSK 消息发送...\n");
            qpsk_tx_update_frame(ad9361_phy, received_cmd);
        }
